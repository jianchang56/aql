...
export const metadata = {
  title: "编写查询",
  description: "查看 AQL 表结构，使用命名参数和有边界的只读 SQL。",
};
...
export default function QueryingPage() {
  return (
    <DocsPage
      ...
      title="先看 schema，再写可靠查询"
      description="不要猜 Agent 私有文件里有哪些字段。先查看 AQL 统一提供的表和字段；AQL 会在执行前拒绝写入操作，并检查敏感字段授权。"
    >
      <DocsSection id="inspect-schema" title="先查看表与字段">
        <CodeBlock
          ai="使用 $aql 列出可以查询的表，查看 sessions schema，并列出内置查询示例与 token-usage 示例。"
          code={[
            "aql schema --list",
            "aql schema sessions",
            "aql examples --list",
      ...
      <DocsSection id="query-shapes" title="可以写哪些只读查询">
        <p>
          支持 SELECT、CTE、WHERE、JOIN、GROUP BY、ORDER BY、LIMIT 和少量明确允许的函数。AQL
          拒绝多语句、DML、DDL、COPY、ATTACH、外部文件、URL 和 shell 插值。
        </p>
        <CodeBlock
          ai="使用 $aql 查询有更新时间的会话，按模型统计数量并降序返回。只使用 AQL 提供的表和字段，不读取 Agent 私有文件。"
          code={[
            "WITH recent AS (",
      ...
      <DocsSection id="bind-parameters" title="绑定用户参数">
        <CodeBlock
      ...
      <DocsSection id="pagination-ordering" title="分页与顺序">
        <CodeBlock
      ...
      <DocsSection id="explain-plan" title="查看授权后的计划">
        <CodeBlock
