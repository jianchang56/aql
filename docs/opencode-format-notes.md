...
Consequently Phase 7 reads `message` + `part` as the pinned 1.17.18 public read projection. It does not read or UNION `session_message` or `event`.
...
`project` and `workspace` are not needed for the Phase 7 canonical mapping. Session Path values come only from explicitly projected `session.directory`/`session.path`; AQL never opens project directories.
...
- `metadata`, `summary_diffs`, `revert`, `permission`, `share_url`: excluded from Phase 7 canonical reads.
...
- `step-finish` parts carry explicit cost/token usage and are the projector authority for session aggregates. Phase 7 emits explicit message usage from assistant message token fields and does not double-count step-finish/session aggregates as additional event facts.
...
## CLI surfaces observed but prohibited for real acceptance

The installed help lists export/import, session, server/web/ACP, provider/auth, plugin, database, GitHub/PR and prompt/run surfaces. Phase 7 does not invoke these against real data. Command availability is not an Action admission contract.

## Action admission survey

The pinned `v1.17.18` official source exposes rename and archive/unarchive through `PATCH /session/:sessionID`, and delete through `DELETE /session/:sessionID`. The relevant evidence is:

- `packages/opencode/src/server/routes/instance/httpapi/groups/session.ts`: `UpdatePayload` contains only the new title, metadata, permission and archive timestamp; the endpoint has no expected revision, ETag, idempotency key or request identity.
- `packages/opencode/src/server/routes/instance/httpapi/handlers/session.ts`: update reads the current session and then independently calls `setTitle`/`setArchived`; it does not bind the mutation to the state observed by AQL.
- `packages/opencode/src/session/session.ts`: `patch` reads current state, constructs a replacement and publishes an update without a compare-and-swap precondition.
- `packages/core/src/session/projector.ts`: the database update predicate is only `session.id = sessionID`; no version/revision participates in the write.
- The generated SDK accepts only session ID plus replacement values. It exposes neither an idempotency mechanism nor an outcome lookup keyed by an AQL action ID.
- Session deletion catches internal removal failures in the session service, while the HTTP handler returns `true`; this cannot provide a stable success mapping for AQL.

Target identity by explicit session ID exists, but atomic expected revision/CAS is absent. Idempotency/outcome lookup and disposable-profile acceptance are also absent, and delete lacks a stable result contract. Therefore `aql-action-opencode` advertises archive, unarchive and rename as unsupported under `opencode-1.17.18-action-survey-v1` with `atomic_precondition_unavailable`. It contains no observer, executor, reconciler or writer path.
...
2. Delete/share are outside the current canonical Action enum. Archive, unarchive and rename are closed as versioned unsupported by the Action survey above.
3. `session_message`/`event`, todos, permissions, shares, account/provider/credential data, reasoning and arbitrary parts/events remain unsupported by this Adapter version.

Any unresolved question keeps the affected canonical capability or Action unsupported.
