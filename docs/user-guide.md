...
AQL 推荐由 AI 使用。先安装 CLI，再安装 AQL Skill，然后直接用自然语言描述问题；需要审计或自动化时，再切换到等价代码。无论入口是什么，AQL 的完整用户路径都一样：选择具体数据库，查询会话、消息或用量等统一表，输出结果。

AQL 进程本身不调用模型或上传数据。调用 AQL 的 Agent 属于另一条信任边界：如果它连接云端模型，提示词和被放入模型上下文的工具结果仍受该 Agent 的隐私设置、企业策略和服务条款约束。不要把“本地查询”等同于“AI 对话一定离线”。
...
```text
使用 $aql 查询 codex 的会话数，按模型分组并按数量降序返回 table。请先确认数据库和可用字段，不要读取敏感内容。
```
