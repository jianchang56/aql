...
在 `P0-T08A/P0-T08B` 中公平比较 GitQL 与 DataFusion，并在 `P0-T09` 选择唯一 MVP 引擎。评分卡不是长期功能愿望清单，而是 Phase 0 可复现的准入测试。
...
512 MiB 是 Phase 0 初始门槛，可根据 fixture record size在 ADR 中调整，但两个候选必须使用同一门槛。
...
5. 两个都失败，停止 `P0-T09`，提出最小第三方案或缩减 SQL 功能，不得修改评分规则让候选勉强通过。
