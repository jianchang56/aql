//! Per-query bounded parse cache for the Kimi Code adapter.
//!
//! One adapter instance serves exactly one query (the CLI rebinds sources per
//! query), so scans of the messages/tool_calls/usage tables used to re-read
//! and re-parse every agent wire file once per table. This module caches the
//! single-pass parse outcome of each wire file in adapter memory for the
...
///
/// Extraction mirrors the pre-cache per-column gating exactly: `content` and
/// `content_json` are separate classes because the wire parser deserializes
/// them through different views with different failure modes. Replay masks
...
impl SensitiveClasses {
    ...
    pub(crate) const CONTENT_JSON: Self = Self(2);
    ...
    fn from_grant(access: AccessGrant) -> Self {
        ...
        if access.content {
            classes = classes.union(Self::CONTENT).union(Self::CONTENT_JSON);
        }
...
pub(crate) fn required_classes(table: TableName, projection: &[ColumnName]) -> SensitiveClasses {
    ...
    match table {
        TableName::Messages => {
            ...
            if projected(projection, "content_json") {
                needed = needed.union(SensitiveClasses::CONTENT_JSON);
            }
        }
        TableName::ToolCalls => {
        ...
        TableName::Usage | TableName::Sessions | TableName::SessionEdges | TableName::Artifacts => {
        }
    }
    needed
}

/// Cache key pinning one wire file at its scan-start watermark.
///
/// `len` is part of the key so an appended wire file misses; `identity`
...
/// One wire file parsed once, fanned out to every wire-backed canonical
/// table, kept for the adapter (query) lifetime.
pub(crate) struct ParsedWireFile {
...
pub(crate) enum CacheLookup {
    Hit(Arc<ParsedWireFile>),
    Miss(SensitiveClasses),
}

struct CacheEntry {
    parsed: Arc<ParsedWireFile>,
...
impl ParseCache {
    ...
    pub(crate) fn insert(&mut self, key: FileCacheKey, parsed: Arc<ParsedWireFile>) {
...
fn estimated_retained_bytes(parsed: &ParsedWireFile) -> usize {
    let mut bytes = std::mem::size_of::<ParsedWireFile>()
        .saturating_add(
            parsed
                .messages
                .capacity()
...
mod tests {
    ...
    fn empty_parsed(extracted: SensitiveClasses) -> Arc<ParsedWireFile> {
        Arc::new(ParsedWireFile {
            messages: Vec::new(),
            tool_calls: Vec::new(),
            usage: Vec::new(),
            extracted,
    ...
    fn hit_requires_the_full_needed_class_set() {
        ...
        let lookup = cache.lookup(&key(identity, 1), SensitiveClasses::TOOL_INPUT);
        assert!(
    ...
    fn full_cache_falls_back_to_reparsing() {
        ...
        aql_test_support::generate_kimi(&fixtures).expect("fixture generator succeeds");
        let root = fixtures.join("full");
        let wire =
            root.join("sessions/wd_workspace_84b61923346a/session-full/agents/main/wire.jsonl");
        let wire_len = std::fs::metadata(&wire).expect("wire metadata").len();

        let run = |adapter: &KimiCodeAdapter| -> u64 {
            let source = adapter
                .probe(&ProbeRequest {
            ...
            for (table, projection) in [
                ...
                (TableName::Usage, &["usage_id", "total_tokens"][..]),
            ] {
        ...
        let cached = KimiCodeAdapter::new(b"fixture-salt".to_vec());
        let cached_bytes = run(&cached);
        let uncached = KimiCodeAdapter::new_with_parse_cache_limit(b"fixture-salt".to_vec(), 1);
        let uncached_bytes = run(&uncached);
        assert_eq!(
            uncached_bytes,
            cached_bytes + wire_len,
