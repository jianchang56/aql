//! Evidence-backed unsupported Action surface for OpenCode 1.17.18.
//!
//! The pinned HTTP and session-store update paths bind targets by session ID,
//! but expose no atomic expected revision, idempotency key, or outcome lookup.
//! This Adapter therefore contains no writer.
...
pub const OPENCODE_ACTION_SURVEY_VERSION: &str = "opencode-1.17.18-action-survey-v1";

#[derive(Clone, Copy, Debug, Default)]
pub struct OpenCodeActionAdapter;

impl OpenCodeActionAdapter {
    #[must_use]
    pub fn capabilities_snapshot() -> Vec<ActionCapability> {
        ...
        .map(|operation| {
            ActionCapability::unsupported(
                operation,
                OPENCODE_ACTION_SURVEY_VERSION,
...
impl AgentActionAdapter for OpenCodeActionAdapter {
...
mod tests {
    ...
    #[test]
    fn all_opencode_mutations_are_versioned_unsupported_without_a_writer() {
        let capabilities = OpenCodeActionAdapter::capabilities_snapshot();
        ...
        assert!(capabilities.iter().all(|capability| {
            !capability.is_supported()
                ...
                && capability.capability_version == OPENCODE_ACTION_SURVEY_VERSION
                && matches!(
                    capability.status,
    ...
    #[test]
    fn unsupported_opencode_adapter_has_no_observe_execute_or_reconcile_path() {
        let adapter = OpenCodeActionAdapter;
        let source = SourceId::new("opencode:synthetic");
        let entity = EntityId::new("opencode:synthetic:session");
        assert!(matches!(
